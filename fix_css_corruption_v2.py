...
try:
    ...
    for i in range(len(lines) - 1, -1, -1):
        if '}' in lines[i] and '.chat-header-small' in "".join(lines[i-10:i]):
            truncate_index = i
            break
            
    if truncate_index != -1:
        print(f"Found truncation point at line {truncate_index + 1}")
        clean_lines = lines[:truncate_index + 1]
...
"""
        with open(file_path, 'w', encoding='utf-8') as f:
            ...
            f.write(clean_css)
        print('Successfully cleaned and updated css/style.css')
    else:
        print('Could not find .chat-header-small block to truncate safely.')
